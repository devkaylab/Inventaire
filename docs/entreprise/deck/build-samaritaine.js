...
async function main() {
  ...
  // optique tombe à H/2, comme celui de l'écran en face.
  {
    ...
    s.addShape('rect', { x: M, y: yb + 0.87, w: 2.2, h: 0.022, fill: { color: P.INK }, line: { color: P.INK, width: 0 } })
    // La baseline, ajoutée par Julien le 9 septembre 2026. C'est la seule
    // phrase de la page, et elle nomme le produit sans rien démontrer.
    s.addText('La fiabilisation du stock au quotidien', { x: M, y: yb + 1.08, w: 5.4, h: 0.4, fontFace: FONT, fontSize: 13, color: P.SLATE, margin: 0 })
  ...
  // sans adjectif. En faire la douleur principale serait faux.
  {
    ...
    d.alineas(s, [
      ['Un service mobilisé du début à la fin.', "Quelqu'un de l'Inventory Control prépare l'inventaire, participe au comptage, puis reprend le rapport d'inventaire pour le recouper avec le stock théorique."],
      ['Une flotte à préparer.', "Les terminaux se chargent, se connectent et se vérifient un par un avant chaque session — date, localisation. C'est un travail en soi. Sans oublier les mises à jour, qui ne sont pas automatiques."],
    ...
    d.encadre(s, 'Ce que ça coûte vraiment', "Ce n'est pas le comptage qui pèse — il est fait par les équipes de vente, et il se fait bien. C'est tout ce qu'il faut monter autour, à chaque fois, et qui dissuade de compter souvent.", { x: M, y: 5.6, w: W - 2 * M, h: 1.15 })
    d.pied(s, 4, PIED)
  ...
  // lisible comme une respiration entre le problème et la réponse.
  {
    ...
    // ⚠️ UNE SEULE PHRASE (Julien, 9 septembre 2026). La version d'avant en
    // portait trois — deux lignes de titre et un renvoi aux pages suivantes —
    // et elle expliquait ce que la suite allait montrer. Une respiration
    // n'annonce rien.
    s.addText("Quantinvo, l’outil d’inventaire qui apporte l’autonomie.", {
      x: M, y: 3.0, w: W - 2 * M, h: 1.6, fontFace: FONTD, fontSize: 32, bold: true,
  ...
  // ════ 6. La réponse, dans l'ordre des irritants ════
  {
    const s = pres.addSlide()
    d.entete(s, 'Avec Quantinvo')
    d.titreLarge(s, 'Le chef d’équipe conduit. Ses collaborateurs comptent.', { y: 1.25, size: 25 })
    d.para(s, "Une application sur les smartphones que vos équipes ont déjà — iOS comme Android —, un tableau de bord pour celui qui pilote, un rapport à la fin. Rien à charger la veille, rien à briefer, aucune flotte à préparer.", { x: M, y: 1.95, w: W - 2 * M, h: 0.4, size: 12, color: P.SLATE })
    await troisEcrans(s, [
      ...
      { ecran: 'comptage', titre: 'Ils comptent au rayon, à la marque, à la référence', texte: "Scanner l'étiquette, puis les articles. Le geste ne demande pas de briefing.", fill: P.TINT },
      { ecran: 'audit', titre: 'Il audite selon les règles de l’Inventory Control', texte: "L’audit se lance de la même application, pendant que le comptage continue.", fill: P.MIST },
    ], { y: 2.5 })
    d.pied(s, 6, PIED)
  ...
  // ════ 8. Ce qui vous revient : le rapport ════
  {
    ...
    d.para(s, "Il part du stock théorique, pas seulement de ce qui a été scanné : un article attendu et jamais trouvé y figure, avec son manque. Le recoupement n'est plus à faire : il se télécharge.", { x: M, y: 2.85, w: COL, h: 1.5, size: 12.5 })
  ...
  // ════ 9. L'inventaire aléatoire ════
  {
    ...
    d.titreLarge(s, "L'inventaire aléatoire, prêt à fonctionner.", { y: 1.25, size: 25 })
    d.para(s, "L’Inventory Control en a posé les objectifs : identifier les vols, tester la rigueur des procédures, garantir que chaque responsable maîtrise son stock. Confier le comptage au floor, c'est exactement cela.", { x: M, y: 1.95, w: W - 2 * M - 2.6, h: 0.6, size: 12, italic: true, color: P.SLATE })
    ...
    d.alineas(s, [
      ['La sélection.', "Elle reste du côté de l’Inventory Control, ou du superviseur curieux : variances négatives répétées, articles sensibles, slow movers."],
      ['Le déclenchement.', "Un inventaire ciblé se crée en quelques minutes — une marque, un rayon, une liste d'articles — le jour même s'il le faut. Les équipes concernées sont invitées depuis Quantinvo."],
  ...
  // pour l'audience qui les demande. Ne pas les réintroduire ici.
  {
    ...
    d.alineas(s, [
      ['Le matériel.', "SmartCount repose sur une flotte de terminaux dédiés : à charger, configurer, vérifier avant chaque session — le travail qu'on cherche justement à ne plus faire. Quantinvo tourne sous iOS et sous Android, sur les téléphones que vos équipes ont déjà."],
      ['La mise à jour.', "Celle des outils Zebra, comme celle de la base, demande l'intervention de l'IT Zebra et de la DSI du magasin. Quantinvo se met à jour tout seul, et les données d'inventaire sont chargées avant chaque comptage par les superviseurs eux-mêmes."],
      ...
      ["L'éditeur.", "SmartCount est un module au catalogue d'un géant du matériel. Quantinvo ne fait qu'une chose — l'inventaire — et la personne qui l'a imaginé répond elle-même à vos questions."],
    ], { x: M, y: 2.05, w: W - 2 * M, h: 3.4, size: 12.5, gap: 13 })
    d.encadre(s, 'Pour être honnête', "Un terminal durci encaisse mieux les chocs qu'un téléphone, et si votre flotte est déjà amortie, l'argument du matériel pèse moins. Ce qui ne bouge pas : un outil que le floor prend en main sans briefing, et un fichier qui arrive déjà croisé.", { x: M, y: 5.1, w: W - 2 * M, h: 1.1 })
    d.pied(s, 10, PIED)
  ...
  // périmé pendant une semaine — voir LISEZMOI.md.
  {
    ...
    grilleOffres(d, s, { x: M, y: 2.55, w: W - 2 * M, h: 3.0, rythme: 'mois', points: false, surtitre: 'ADAPTÉE AU MAGASIN' })
    d.alineas(s, [
      ["Ce que ça change pour l'inventaire.", "À l'intérieur d'un palier, compter davantage ne coûte rien : un chef d'équipe qui décide de compter son rayon un mardi matin n'a aucun budget à demander. C'est ce qui rend le transfert au floor tenable dans la durée. L'application, le tableau de bord, les rapports et les mises à jour sont compris."],
  ...
  // ════ 12. Pour finir ════
  {
    const s = pres.addSlide()
    d.finale(s, {
      titre: 'Un essai suffit pour être convaincu.',
      texte: "Une démonstration avec un chef d'équipe et deux vendeurs, sur votre propre stock théorique : il l'importe, ils balisent, ils comptent, et vous recevez le rapport. Un mardi matin avant l'ouverture suffit.",
