...
export function PageInscription() {
  ...
  const edge = useCallback(async (corps: Record<string, unknown>) => {
    ...
    if (error) {
      ...
      return { success: false, error: t('Le service n’a pas répondu. Réessayez dans un instant.') }
    }
    return data
  }, [])
  ...
  const creerCompte = async () => {
    ...
    if (!r?.success) { setOccupe(false); setErreur(r?.error ?? t('Création impossible.')); return }
    const { error } = await supabase.auth.signInWithPassword({
      email: email.trim().toLowerCase(), password: motDePasse,
    })
    setOccupe(false)
    if (error) { setErreur(t('Compte créé. Connectez-vous pour continuer.')); return }
    setMotDePasse('')
    setEtape(4)
  }
  ...
  const lignes = useMemo(() => magasins.map((m, i) => {
    const n = appareilsDe(m)
    return {
      nom: m.nom.trim() || t('Magasin %{n}', { n: i + 1 }),
      appareils: n,
      offre: n ? nomOffre(n) : '',
      ...
      <>
        ...
        <main className="wrap ins-page">
          <section className="ins-carte ins-fin">
            <h1>{t('L’inscription en ligne ouvre bientôt')}</h1>
            ...
            <p className="muted">
              {t('Quantinvo fonctionne, et ses tarifs sont publics — mais la souscription en ligne n’est pas encore ouverte.')}
              {ecrivezNous()
                ? <> {t('Écrivez-nous : nous ouvrons vos accès à la main, avec la même offre.')}</>
                : <> {t('Elle ouvre très bientôt.')}</>}
            </p>
  ...
  if (!prete) {
    return (
      <>
        <SiteHeader langue={langue} />
        <main className="wrap ins-page"><p className="muted">{t('Chargement…')}</p></main>
        <SiteFooter langue={langue} />
      </>
    )
  }
  ...
  return (
    <>
      ...
      <main className="wrap ins-page">
        {paye ? (
          <section className="ins-carte ins-fin">
            <h1>{t('Votre espace est prêt')}</h1>
            <p className="muted">
              {t('Le paiement est passé. Vos magasins sont créés et vous en êtes l’administrateur — vous pouvez constituer votre équipe et lancer un premier inventaire.')}
            </p>
            <Link href="/entreprise" className="btn btn-primary btn-block">{t('Ouvrir mon espace')}</Link>
          </section>
        ) : (
          <section className="ins-carte">
            ...
            {etape === 1 && (
              <>
                ...
                <p className="muted">
                  {t('Quantinvo se prend en ligne, sans devis : vous répondez à quelques questions, vous voyez votre offre, vous réglez. Vos accès s’ouvrent aussitôt.')}
                </p>
                <div className="field">
                  <label htmlFor={`${uid}-email`}>{t('Adresse e-mail')}</label>
                ...
                <button type="button" className="btn btn-primary btn-block" disabled={occupe || !email.trim()}
                  ...
                  {occupe ? t('Envoi…') : t('Recevoir mon code')}
                </button>
                <MentionCollecte finalite={t('créer votre compte, établir votre offre et vous adresser votre facture')} />
              </>
            )}
            ...
            {etape === 2 && (
              <>
                <h1>{t('Votre code')}</h1>
                <p className="muted">
                  {t('Nous avons envoyé un code à six chiffres à')} <b>{email}</b>{t('. Il est valable dix minutes.')}
                </p>
                <div className="field">
                  <label htmlFor={`${uid}-code`}>{t('Code à six chiffres')}</label>
            ...
            {etape === 3 && (
              <>
                ...
                <button type="button" className="btn btn-primary btn-block"
                  ...
                  {occupe ? t('Création…') : t('Créer mon compte')}
                </button>
              </>
            )}
            ...
            {etape === 4 && (
              <>
                <h1>{t('Où en êtes-vous de vos inventaires ?')}</h1>
                  ...
                  {([['oui', 'Nous faisons des inventaires régulièrement', 'Vous savez ce que vous comptez et à quel rythme.'],
                    ...
                    .map(([v, titre, sous]) => (
                      <button key={v} type="button"
                        ...
                        <span className="s">{t(sous)}</span>
                      </button>
                    ))}
                </div>
                <button type="button" className="btn btn-primary btn-block" disabled={!pratique}
                        onClick={() => avancer(5)}>{t('Continuer')}</button>
              </>
            )}

            {etape === 5 && (
              <>
                <h1>{pratique === 'oui' ? t('Comment vous comptez aujourd’hui') : t('Ce que vous aimeriez mettre en place')}</h1>
                <div className="field">
                  <label htmlFor={`${uid}-freq`}>
                    {pratique === 'oui' ? t('À quelle fréquence comptez-vous ?') : t('À quelle fréquence souhaitez-vous compter ?')}
                  </label>
                  <select id={`${uid}-freq`} value={frequence} onChange={(e) => setFrequence(e.target.value)}>
                    <option value="">{t('Choisir…')}</option>
                    {FREQUENCES.map((f) => <option key={f.valeur} value={f.valeur}>{t(f.libelle)}</option>)}
                  </select>
                </div>
                <div className="field">
            ...
            {etape === 6 && (
              <>
                <h1>{t('Vos magasins')}</h1>
                <p className="muted">{t('Une licence par magasin. Vous pourrez en ajouter à tout moment.')}</p>
                {magasins.map((m, i) => (
                  <div key={i} className="ins-magasin">
                    <div className="field">
                      <label htmlFor={`${uid}-mag${i}`}>{t('Magasin %{n}', { n: i + 1 })}</label>
                    ...
                    <div className="field">
                      ...
                      <select id={`${uid}-app${i}`} value={m.tranche}
                        ...
                        <option value="">{t('Choisir…')}</option>
                        {APPAREILS_TRANCHES.map((tr) =>
                          <option key={tr.valeur} value={tr.valeur}>{t(tr.libelle)}</option>)}
                      </select>
                      {m.tranche === 'exact' && (
                        <input className="ins-exact" type="number" min={101} step={1} value={m.exact}
                               placeholder={t('Nombre exact')}
                               onChange={(e) => setMagasins((v) =>
                                 v.map((x, j) => j === i ? { ...x, exact: e.target.value } : x))} />
                      )}
                      {refusMagasin(m) && m.tranche !== '' && (
                        <p className="offre-refus" role="status">{t(refusMagasin(m) ?? '')}</p>
                      )}
                    </div>
                    {magasins.length > 1 && (
                      <button type="button" className="link-btn danger-link"
                              onClick={() => setMagasins((v) => v.filter((_, j) => j !== i))}>
                        {t('Retirer ce magasin')}
                      </button>
                    )}
                  </div>
                ))}
                <p className="field-hint">
                  {t('Un téléphone par personne qui compte au même moment — les comptes, eux, sont illimités. C’est ce chiffre qui détermine l’offre, magasin par magasin.')}
                </p>
                ...
                <button type="button" className="btn btn-primary btn-block"
                        ...
                        onClick={() => avancer(7)}>{t('Continuer')}</button>
              </>
            )}

            {etape === 7 && (
              <>
                <h1>{t('Votre entreprise')}</h1>
                <div className="field">
                ...
                <div className="field">
                  <label htmlFor={`${uid}-tel`}>{t('Téléphone (facultatif)')}</label>
            ...
            {etape === 8 && (
              <>
                <h1>{t('Voici votre offre, magasin par magasin')}</h1>
                <div className="ins-recap">
                  {lignes.map((l, i) => (
                    <div className="l" key={i}>
                      <span>{l.nom}{l.appareils ? ` · ${t('%{n} appareils', { n: nb(l.appareils) })}` : ''}</span>
                ...
                <div className="ins-rythmes">
                  {(['monthly', 'yearly'] as const).map((r) => (
                    <button key={r} type="button"
                            ...
                            onClick={() => setRythme(r)}>
                      <span className="t">{r === 'monthly' ? t('Au mois') : t('À l’année')}</span>
                      <span className="p">
                        {r === 'monthly'
                ...
                <Link href={lien('/')} className="link-btn ins-lien">{t('J’ai besoin de réfléchir')}</Link>
                <p className="field-hint">
                  {t('Rien ne se perd : nous gardons vos réponses trente jours et vous reprendrez où vous en êtes.')}
                </p>
              </>
            )}
